...
export function PurchaseDocumentAnalysisReview({ analysis, purchase, currency = 'EUR', error, isPending = false, isStale = false, disabled = false, onSubmit, onCancel, onDirtyChange }) {
  ...
  return <form aria-busy={pending} aria-label="Revisar datos detectados" className="min-w-0 space-y-6 [overflow-wrap:anywhere]" noValidate onSubmit={submit} ref={formRef}>
    ...
    <fieldset className="min-w-0 space-y-4" disabled={pending || disabled}>
      ...
      <p className="text-sm leading-6 text-text-muted">Los {purchase.items?.length ?? 0} productos existentes se conservan, con sus garantías y documentos. Aquí corriges los productos detectados; puedes añadirlos como nuevos, sin reemplazar los existentes.</p>
      ...
      <CheckField error={errors.applyItems?.message} help="Se añadirán como nuevos productos. No se borrarán ni reemplazarán los que ya tienes. Revisa que no estén duplicados." label="Añadir estos productos a la compra" {...register('applyItems')} />
    </fieldset>
