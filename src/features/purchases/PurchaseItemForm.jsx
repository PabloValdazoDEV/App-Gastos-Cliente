...
export function PurchaseItemForm({ initialItem = null, purchaseDate, onSubmit, onCancel, isPending = false, error = null, currency = 'EUR' }) {
  ...
  return (
    <form aria-busy={pending} aria-label={initialItem ? 'Editar producto' : 'Añadir producto'} className="min-w-0 max-w-full space-y-5" noValidate onSubmit={submit} ref={formRef}>
      <PurchaseItemFields currency={currency} disabled={pending} errors={errors} purchaseDate={purchaseDate} register={register} watch={watch} />
